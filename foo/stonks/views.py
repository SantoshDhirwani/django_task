...
def refresh_view(request):
    for stonk in Stonk.objects.select_for_update(nowait=False).all():
...
def top_view(request):
    for stonk in Stonk.objects.select_for_update(nowait=False).filter(value__gt=25000):
        bump_stonk(stonk)

    for stonk in Stonk.objects.select_for_update(nowait=False).filter(value__lt=25000):
        hump_stonk(stonk)
